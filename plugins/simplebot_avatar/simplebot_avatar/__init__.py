...
class Avatar(Plugin):
    ...
    def activate(cls, bot):
        ...
        cls.commands = [
            PluginCommand('/avatar', ['[text]'], _('Generates an avatar based on the given text, if no text is given a random avatar is generated'), cls.avatar_cmd)]
        cls.bot.add_commands(cls.commands)

    @classmethod
    def avatar_cmd(cls, ctx):
        chat = cls.bot.get_chat(ctx.msg)
        url = 'https://www.peppercarrot.com/extras/html/2016_cat-generator/'
        if not ctx.text:
            with requests.get(url+'index.php', headers=HEADERS) as r:
                r.raise_for_status()
